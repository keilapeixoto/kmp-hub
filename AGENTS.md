...
Migrada do protótipo standalone `KMP_Ficha_de_Consulta.html` (salvava só em
localStorage) para dentro do Hub: aba "Fichas" no perfil do cliente
(`/clientes/[id]?tab=fichas`) + editor completo em `/fichas/[id]`
(`consultation_forms`, RLS por `clients.consultor_id`, mesmo padrão de
`client_relations`). Autosave debounced, exportação por impressão do
navegador (sidebar/header somem via `print:hidden`).

Inclui preenchimento automático a partir de transcrição colada, via API da
Anthropic (`ANTHROPIC_API_KEY`, só servidor —
`app/(staff)/fichas/actions.ts`, `extractConsultationDataFromTranscript`).
Isso antecipa uma peça da Fase 4 (IA) do roadmap — decisão explícita da
cliente, não um desvio silencioso. Campos preenchidos por IA ficam marcados
com selo "gerado por IA — revisar" (`ai_pending_sections`) até o consultor
editar ou confirmar cada seção, seguindo a regra do projeto de nunca aceitar
conteúdo de IA sem validação humana.
