// Etapa 2 da importação da pasta de controle: lê o CSV de mapeamento
// revisado (etapa 1, scan-pasta-controle.mjs), cria os clientes que não
// existem e sobe os arquivos para o bucket privado "documents", registrando
// cada um na tabela documents (categoria = pasta de topo de origem).
//
// Mesmo padrão de import-key-migration.mjs. Usa a SUPABASE_SECRET_KEY do
// .env.local (somente servidor — este script roda localmente, nunca no
// navegador). Idempotente: pula arquivos cujo storage_path já existe em
// documents, então pode rodar de novo sem duplicar.
//
// Uso:
//   node scripts/import-pasta-controle.mjs "<pasta base>" [mapa.csv] [--dry-run] [--owner-email=email]
//
// Rode sempre primeiro com --dry-run e confira o relatório antes de subir de
// verdade. O "dono" (enviado_por + consultor_id dos clientes criados) é o
// usuário de auth com o e-mail informado (padrão: keila.peixoto@kmpconsulting.com.au).
...
const CSV = process.argv.find((a, i) => i >= 3 && !a.startsWith("--")) ?? "import/pasta-controle-map.csv";
...
if (!BASE) {
  console.error('Uso: node scripts/import-pasta-controle.mjs "<pasta base>" [mapa.csv] [--dry-run]');
  process.exit(1);
}
...
const rows = readFileSync(CSV, "utf8")
  ...
  .filter(Boolean)
  .map((line) => {
    const [caminho, categoria, cliente, qtd, acao] = line.split(";");
    return { caminho, categoria, cliente, qtd: Number(qtd), acao };
  })
